...
function getLastUser(): string {
  ...
  const match = document.cookie.match(/timehit_last_user=([^;]+)/);
  return match ? decodeURIComponent(match[1]) : "";
}

function setLastUser(email: string) {
  document.cookie = `timehit_last_user=${encodeURIComponent(email)};path=/;max-age=${60 * 60 * 24 * 365}`;
}

export default function LoginPage() {
  const lastUser = getLastUser();
  const [email, setEmail] = useState(lastUser || "admin@timehit.com");
