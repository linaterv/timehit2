...
- Tabs:
  ...
  - **Documents** — file list with per-document visibility badges (**Client** purple, **Contractor** blue). Click row → edit dialog (label, visibility checkboxes, Save/Cancel). "Upload Document" button → dialog: pick file, set label + visibility checkboxes, Save/Cancel. Delete with confirmation. **Admin/broker** can upload/edit/delete. **Contractor** sees only docs marked `visible_to_contractor`. **Client** sees only docs marked `visible_to_client` (and only if placement `client_can_view_documents` is on). Backend: `PlacementDocument` has `visible_to_client` + `visible_to_contractor` booleans (default false). PATCH accepts label + visibility fields.
